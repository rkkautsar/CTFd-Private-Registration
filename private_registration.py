...
from flask import current_app as app, jsonify, redirect, render_template, render_template_string, request, send_file, session, url_for
...
def add_to_invitation_list(csv_file):
    ...
    for team in team_list:
        ...
        registered_names = Teams.query.add_columns('name').filter_by(name=name).first()
        registered_emails = Teams.query.add_columns('email').filter_by(email=email).first()
        invited_names = InvitedTeams.query.add_columns('name').filter_by(name=name).first()
        invited_emails = InvitedTeams.query.add_columns('email').filter_by(email=email).first()
        ...
        if len(errors) == errors_len:  # No additional errors at this point, invite new team
    ...
    if number_of_invited_teams > 0:
        messages.append('Succesfully added %d teams to invitation list' % number_of_invited_teams)
...
def override_register_template():
    ...
    if selected_option == 'token':
        register_template_path = '{}/templates/private-registration-token.html'.format(dir_path)
    elif selected_option == 'email':
        register_template_path = '{}/templates/private-registration-email.html'.format(dir_path)

    if register_template_path:
        with open(register_template_path, 'r') as register_template:
...
def load(app):
    ...
    def get_invited_teams():
        ...
        for team in invited_teams:
            if team.name in registered_teams:
                ...
                registered = False
            json['invited_teams'].append({'id': team.id, 'name': team.name, 'email': team.email,
    ...
    def export_csv():
        ...
        return send_file(output, as_attachment=True, attachment_filename=full_name)
        ...
        if utils.can_send_mail():
            ...
            for team in invited_teams:
                ...
                if selected_option == 'token':
                    ...
                    token_text = ''
                text = """Team {} ({}) is invited for {}. You can register in {}{}.""".format(
                    team.name,
                    team.email,
    ...
    @app.route('/admin/invited_teams/send_invitation/<int:id>', methods=['POST'])
    @admins_only
    def send_invitation(id):
        if utils.can_send_mail():
            selected_option = utils.get_config('private_registration_option')
            team = InvitedTeams.query.add_columns('name', 'email', 'token').filter_by(id=id).first()
    ...
    def private_register():
        ...
        if request.method == 'POST':
            ...
            if selected_option == 'token':
                ...
                invited_team = InvitedTeams.query.add_columns('name',
                                                              'email').filter_by(token=token).first()
                if not invited_team:
                    errors.append('Invalid token')
            elif selected_option == 'email':
                email = request.form['email']
                invited_team = InvitedTeams.query.add_columns('name',
                                                              'email').filter_by(email=email).first()
                if not invited_team:
            ...
            if len(errors) == 0:
                team = Teams.query.add_columns('id').filter_by(name=invited_team.name).first()
                if team:
            ...
            logger = logging.getLogger('regs')
            logger.warn("[{0}] {1} registered with {2}".format(time.strftime("%m/%d/%Y %X"),
                                                               name.encode('utf-8'),
                                                               email.encode('utf-8')))
