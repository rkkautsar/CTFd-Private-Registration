...
def add_to_invitation_list(csv_file):
    ...
    for team in team_list:
        ...
        registered_names = Teams.query.add_columns(
            'name').filter_by(name=name).first()
        registered_emails = Teams.query.add_columns(
            'email').filter_by(email=email).first()
        invited_names = InvitedTeams.query.add_columns(
            'name').filter_by(name=name).first()
        invited_emails = InvitedTeams.query.add_columns(
            'email').filter_by(email=email).first()
    ...
    if number_of_invited_teams > 0:
        messages.append(
            'Succesfully added %d teams to invitation list' % number_of_invited_teams)
...
def override_register_template():
    ...
    if selected_option == 'token':
        register_template_path = '{}/templates/private-registration-token.html'.format(
            dir_path)
    elif selected_option == 'email':
        register_template_path = '{}/templates/private-registration-email.html'.format(
            dir_path)
            ...
            if len(errors) > 0:
                ...
                with app.app_context():
                    ...
                    if utils.can_send_mail() and utils.get_config('verify_emails'):
                        ...
                        if utils.can_send_mail():
                            utils.sendmail(email, "You've successfully registered for {}".format(
                                utils.get_config('ctf_name')))
